...
async def profile_read(request: Request, message: str=None): 
    ...
    readings = conn.execute("SELECT *FROM instantaneous_profile_readings").fetchall()     
    conn.close()
    return templates.TemplateResponse("instant_profile_read.html", {"request": request, "readings":readings,"message":message})   
